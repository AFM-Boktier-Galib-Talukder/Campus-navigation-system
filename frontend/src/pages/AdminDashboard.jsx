...
function AdminDashboard() {
  ...
  useEffect(() => {
    const fetchFloorDots = async () => {
      setIsLoading(true);
      try {
        const response = await fetch(`http://localhost:1490/api/admin/floor/dots`);
        if (response.ok) {
          const data = await response.json();
          setFloorDots(data.data || []);
          setFilteredDots(data.data || []);
  ...
  useEffect(() => {
    if (!searchQuery.trim()) {
      ...
      const query = searchQuery.toLowerCase();
      const filtered = floorDots.filter(
        (dot) => (dot.left && dot.left.toLowerCase().includes(query)) || (dot.right && dot.right.toLowerCase().includes(query))
      );
      setFilteredDots(filtered);
    }
  }, [searchQuery, floorDots]);
  ...
  const handleSaveDot = async (formData) => {
    try {
      const response = await fetch(`http://localhost:1490/api/admin/floor/dots/${selectedDot.dot}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (response.ok) {
        const updatedData = await response.json();
        // Update the local state
        setFloorDots((prev) => prev.map((dot) => (dot.dot === selectedDot.dot ? updatedData.data : dot)));
        setIsModalOpen(false);
        setSelectedDot(null);
      } else {
        const errorData = await response.json();
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-green-50/90 to-emerald-50/90 font-inria">
            ...
            <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-8 mb-6">
              ...
              <div className="flex-shrink-0">
                <div className="bg-gradient-to-r from-green-100 to-emerald-100 border border-green-200 rounded-lg px-4 py-3 flex items-center gap-3 shadow-sm hover:shadow-md transition-all duration-200 mb-3 w-fit">
                  ...
                  <span className="text-green-800 font-medium text-sm whitespace-nowrap">💡 Click on any card below to edit the dot information</span>
                </div>
                <div className="text-gray-600 text-sm pl-2">
                  {filteredDots.length} of {floorDots.length} dots
                  {searchQuery && ` matching "${searchQuery}"`}
          ...
          <div className="mb-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {filteredDots.map((dot) => (
                  ...
                  >
                    {/* Dot Title */}
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="text-xl font-bold text-green-600">Dot {dot.dot}</h3>
                      <div className="w-8 h-8 bg-gradient-to-br from-green-400 to-emerald-400 rounded-lg flex items-center justify-center shadow-md group-hover:shadow-lg transition-all">
                    ...
                    {/* Connections */}
                    <div className="mb-3">
                      <p className="text-sm font-semibold text-gray-700 mb-1">Connections:</p>
                      <div className="flex flex-wrap gap-1">
                        {dot.connection && dot.connection.length > 0 ? (
                          dot.connection.map((conn, idx) => (
                          ...
                          <span className="text-gray-500 text-xs italic">No connections</span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-3">
                      <div>
                        ...
                        <p className="text-sm text-gray-800 bg-gray-50 px-2 py-1 rounded min-h-[24px]">
                          {dot.left || <span className="text-gray-400 italic">Empty</span>}
                        </p>
                      </div>
                      <div>
                        <p className="text-xs font-semibold text-gray-600 mb-1">Right:</p>
                        <p className="text-sm text-gray-800 bg-gray-50 px-2 py-1 rounded min-h-[24px]">
                          {dot.right || <span className="text-gray-400 italic">Empty</span>}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <p className="text-xs font-semibold text-gray-600 mb-1">Up:</p>
                        <p className="text-sm text-gray-800 bg-gray-50 px-2 py-1 rounded min-h-[24px]">
                          {dot.up !== null ? dot.up : <span className="text-gray-400 italic">null</span>}
                        </p>
                      </div>
                      <div>
                        <p className="text-xs font-semibold text-gray-600 mb-1">Down:</p>
                        <p className="text-sm text-gray-800 bg-gray-50 px-2 py-1 rounded min-h-[24px]">
                          {dot.down !== null ? dot.down : <span className="text-gray-400 italic">null</span>}
                        </p>
                      </div>
                    </div>
                  </div>
            ...
            {!isLoading && filteredDots.length === 0 && (
              <div className="text-center py-12">
                ...
                <p className="text-gray-500">{searchQuery ? `No rooms found matching "${searchQuery}"` : "No floor data available"}</p>
              </div>
            )}
          </div>
        </div>
      ...
      {/* Edit Modal */}
      <DotEditModal isOpen={isModalOpen} onClose={handleCloseModal} dotData={selectedDot} onSave={handleSaveDot} />
    </div>
  );
}
